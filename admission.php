...
if ($res->num_rows > 0) {
    // Already submitted → go to payment page
    header("Location: admission-fee-payment.php");
    exit();
}
$stmt->close();
...
================================ */
$room_number = $existing_admission['room_number'] ?? ('RM-' . rand(100,999));
...
if ($stmt->execute()) {
    ...
    exit();
        ...
        if ($stmt->execute()) {
            // Redirect to admission fee payment page so users can immediately pay the admission fee
            ...
            <form method="POST" action="">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="email" name="email" class="form-control" value="<?php echo htmlspecialchars($existing_admission['email'] ?? ''); ?>" required>
                    </div>
                </div>
